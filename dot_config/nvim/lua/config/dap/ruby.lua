--- NOTE: Adapted from https://github.com/suketa/nvim-dap-ruby/blob/main/lua/dap-ruby.lua
local M = {}
...
local function pick_port()
  local port
  vim.ui.input({ prompt = "Select port to connect to: " }, function(input) port = input end)
  return port
end
...
local function find_cmd_dir(cmd)
  ...
  if vim.fn.executable(cmd) == 1 then return filepath end
  while filepath ~= "" and filepath ~= "/" do
    if vim.fn.executable(filepath .. "/" .. cmd) == 1 then return filepath end
...
local function setup_ruby_adapter(dap)
  dap.adapters.ruby = function(callback, config)
    ...
    vim.defer_fn(function() callback { type = "server", host = server, port = port } end, waiting)
  end
end
...
local function setup_ruby_configuration(dap)
  ...
  local function extend_base_config(config) return vim.tbl_extend("force", base_config, config) end
  local function extend_run_config(config) return vim.tbl_extend("force", run_config, config) end
